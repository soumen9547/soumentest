...
export function useDrop(options: DropAreaOptions): [DropProps, DropAreaState] {
  ...
  const onDrop = useCallback((dataTransfer: DataTransfer, event: React.DragEvent | React.ClipboardEvent) => {
    ...
    } else if (dataTransfer.files && dataTransfer.files.length > 0 && optionsRef.current.onFiles) {
      optionsRef.current.onFiles(Array.from(dataTransfer.files), event as React.DragEvent);
    } else if (dataTransfer.items && dataTransfer.items.length && optionsRef.current.onText) {
