...
const SubsessionCreate = (props: SubsessionCreateProps) => {
  ...
  const onRadioChange = useCallback((event: RadioChangeEvent) => {
    const selectedPattern = event.target.value as keyof typeof SubsessionAllocationPattern;
    setPattern(SubsessionAllocationPattern[selectedPattern]);
  }, []);

  const onInputChange = useCallback((value: number | null | undefined) => {
  ...
  return (
    <div className="room-create">
      ...
      <Radio.Group value={pattern} onChange={onRadioChange} className="room-create-option">
        <Radio value={SubsessionAllocationPattern.Automatically}>Assign automatically</Radio>
        <Radio value={SubsessionAllocationPattern.Manually}>Assign manually</Radio>
      </Radio.Group>
