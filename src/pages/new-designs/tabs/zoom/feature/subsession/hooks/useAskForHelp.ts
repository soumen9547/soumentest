...
const { confirm } = Modal;
interface AskForHelpResponsePayload {
  result: AskHostHelpResponse;
}
interface AskForHelpPayload {
  userId: number;
  displayName: string;
...
export function useAskForHelp(zmClient: ZoomClient, ssClient: SubsessionClient | null) {
  ...
  const onAskForHelpResponse = useCallback((payload: AskForHelpResponsePayload) => {
