...
const AudioVideoStatisticModel = (props: AudioVideoStatisticModelProps) => {
  ...
  const onAudioStatisticChange = useCallback((payload: StatisticPayload) => {
    const {
      data: { encoding, ...restProps }
    } = payload;
    if (encoding) {
      setAudioEncodingStatistic({
        sample_rate: restProps.sample_rate,
        rtt: restProps.rtt,
        jitter: restProps.jitter,
        avg_loss: restProps.avg_loss,
        max_loss: restProps.max_loss
      });
    } else {
      setAudioEncodingStatistic({
        sample_rate: restProps.sample_rate,
  ...
  const onVideoStatisticChange = useCallback((payload: StatisticPayload) => {
    const {
      data: { encoding, ...restProps }
    } = payload;
    if (encoding) {
      setVideoEncodingStatistic({
        sample_rate: restProps.sample_rate,
        rtt: restProps.rtt,
        jitter: restProps.jitter,
        avg_loss: restProps.avg_loss,
