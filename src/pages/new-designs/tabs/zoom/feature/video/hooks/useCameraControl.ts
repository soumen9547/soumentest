...
export function useCameraControl(zmClient: ZoomClient, mediaStream: MediaStream | null) {
  ...
  const onReceiveFarEndControl = useCallback(
    ({
      ...
      userId: number;
      displayName: string;
      currentControllingUserId: number | undefined;
      ...
      if (currentControllingUserId !== undefined) {
        message = `${displayName} wants to take over the control of ${currentControllingDisplayName}?`;
      }
      Modal.confirm({
  ...
  const onReceiveFarEndControlResponse = useCallback(
    ({ isApproved, userId, displayName }: { isApproved: boolean; userId: number; displayName: string }) => {
  ...
  const onCameraInControlChange = useCallback(({ isControlled, userId }: { isControlled: boolean; userId: number }) => {
    if (isControlled) {
      message.info('Your camera is controlled by someone else');
    } else {
  ...
  const onCameraCapabilityChange = useCallback(
    ({ userId, ptz }: { userId: number; ptz: { pan: boolean; tilt: boolean; zoom: boolean } }) => {
      setCameraCapability(
        produce((draft) => {
